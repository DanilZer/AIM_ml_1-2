...
class RandomForest:
    ...
    def predict_proba(self, x):
        ...
        n_objects = x.shape[0]
        n_classes = max(estimator.n_classes if hasattr(estimator, 'n_classes') else estimator.n_classes_ for estimator in self.estimators)
        probas = np.zeros((n_objects, n_classes))

        for i in range(len(self.estimators)):
            estimator = self.estimators[i]
            feature_indices = self.indices[i]
            x_sample = x[:, feature_indices]
            probas += estimator.predict_proba(x_sample)
